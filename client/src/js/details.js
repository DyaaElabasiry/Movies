...
window.onload = function() {
  ...
  xhr.addEventListener('readystatechange', function () {
    if (xhr.readyState === 4) {
      ...
      document.getElementById('title').innerText = response.title || response.name;
      document.getElementById('year').innerText = `(${(response.release_date || response.first_air_date).slice(0, 4)})`;
      addSpecs(response);
...
function addSpecs(response) {
  ...
  specs.innerText += response.release_date || response.first_air_date;
  specs.innerText += ` (${response.origin_country ? response.origin_country[0] : response.original_language}) `;
