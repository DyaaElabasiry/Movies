...
document
  .getElementById("closeVideoButton")
...
search.addEventListener("input", () => {
  ...
  searchTimeout = setTimeout(() => {
    ...
    if (searchAction) {
      ...
      Promise.all([
        fetch(
          `${search_Movie_Url}&query=${encodeURIComponent(searchAction)}`
        ).then((res) => res.json()),
        fetch(
          `${search_Series_Url}&query=${encodeURIComponent(searchAction)}`
        ).then((res) => res.json()),
      ])
        .then(([moviesData, seriesData]) => {
          // Combine results
